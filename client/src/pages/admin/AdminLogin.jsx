...
const AdminDashboard = () => {
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      const response = await api.delete('/upload/blog-image', {
        data: { fileName: blogFormData.image_key }
      })
  ...
  const handleBlogSubmit = async (e) => {
    ...
    try {
      ...
      const blogData = {
        ...blogFormData,
        tags: tagsArray,
        // Ensure we don't send empty image_key if no image
        ...(!blogFormData.image_url && { image_key: null })
      }
  ...
  const handleTestimonialSubmit = async (e) => {
    ...
    try {
      const videoId = extractVideoId(testimonialFormData.youtube_url)
      if (!videoId && testimonialFormData.youtube_url) {
      ...
      if (editingTestimonial) {
        const response = await api.put(`/testimonials/${editingTestimonial.id}`, testimonialData)
        if (response.data.success) {
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center">
              <button
                ...
                className="p-2 rounded-md text-gray-500 hover:text-gray-700 flex"
              >
      ...
      <div className="flex pt-16">
        ...
        <motion.aside
          ...
          className={`fixed lg:relative inset-y-0 left-0 z-40 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0`}
        ...
        {/* Main Content */}
        <main className={`flex-1 p-4 lg:p-6 overflow-x-auto transition-all duration-300 ${sidebarOpen ? 'lg:ml-64' : ''}`}>
          {/* Dashboard Tab */}
          {activeTab === 'dashboard' && (
                  ...
                  {(imagePreview || blogFormData.image_url) && (
                    <div className="mb-4">
                      ...
                      <div className="mt-2 text-sm text-gray-500">
                        Current image: {blogFormData.image_key ? 'Uploaded' : 'External URL'}
                      </div>
                    </div>
                  )}
...
// Reusable Table Components with Working Status Updates
